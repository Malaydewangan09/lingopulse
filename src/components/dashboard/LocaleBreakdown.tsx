    ...
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {(['coverage', 'quality', 'missing'] as SortKey[]).map(s => (
              <button key={s} onClick={() => setSort(s)} style={{
                ...
                borderColor: sort === s ? 'var(--border-bright)' : 'var(--border)',
                background: sort === s ? 'var(--card-hover)' : 'transparent',
      ...
      {locales.length > 8 && (
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.color = 'var(--text-1)'; e.currentTarget.style.background = 'var(--card-hover)'; }}
